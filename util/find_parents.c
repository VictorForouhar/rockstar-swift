...
#define EXTRA_HALO_INFO int64_t descid, np; \
  float alt_m[4], J[3], spin, bullock_spin, Xoff, Voff, \
    ...
    A500[3], m_pe_b, m_pe_d, type, sm, gas, bh_mass, bound_mass;
#include "read_tree.h"
...
void read_hlist(char *filename) {
  ...
  void *data[NUM_INPUTS] = {&(h.id),
    ...
    &(h.m_pe_d), &(h.type), &(h.sm), &(h.gas), &(h.bh_mass), &(h.bound_mass)};
  ...
  for (n=0; n<all_halos.num_halos; n++) {
    ...
    printf("%"PRId64" %"PRId64" %.3e %.2f %.2f %.3f %.3f %"PRId64" %.5f %.5f %.5f %.2f %.2f %.2f %.3e %.3e %.3e %.5f %.5f %.4e %.4e %.4e %.4e %.4e %.5f %.2f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.4f %.3e %.3e %.0f %.3e %.3e %.3e %.4e %"PRId64"\n",
    ...
    th->m_pe_b, th->m_pe_d, th->type, th->sm, th->gas, th->bh_mass, th->bound_mass, th->pid);
  }
}
